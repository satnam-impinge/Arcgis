...
//            mLevelsStart = minLevelCur.getInt(0);
//        }

        Log.i("TAG", "Max levels = " + Integer.toString(mLevels));


        double[] resolution = new double[mLevels - mLevelsStart];
        double[] scale = new double[mLevels - mLevelsStart];
        for (int i = 0; i < mLevels - mLevelsStart; i++) {
         ...
         */
        TileInfo ti = new TileInfo(origin, scale, resolution, mLevels - mLevelsStart, 96, 256, 256);
...
//        Cursor imageCur = mapDb.rawQuery("SELECT tile_data FROM tiles WHERE zoom_level = " + Integer.toString(level), null);
...
//        Cursor imageCur = mapDb.rawQuery("SELECT tile_data FROM tiles", null);
